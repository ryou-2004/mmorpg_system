class Admin::JobStatsController < ApplicationController
  ...
  def index
    ...
    if params[:levels].present?
      levels = params[:levels].map(&:to_i).select { |l| l > 0 && l <= 100 }
    else
  ...
  def level_samples
    ...
    # 各ステータスでのランキング情報も追加
    rankings = {
      hp: stats_data.sort_by { |data| -data[:stats][:hp] }.first(3),
      mp: stats_data.sort_by { |data| -data[:stats][:mp] }.first(3),
      attack: stats_data.sort_by { |data| -data[:stats][:attack] }.first(3),
      defense: stats_data.sort_by { |data| -data[:stats][:defense] }.first(3),
      magic_attack: stats_data.sort_by { |data| -data[:stats][:magic_attack] }.first(3),
      magic_defense: stats_data.sort_by { |data| -data[:stats][:magic_defense] }.first(3),
      agility: stats_data.sort_by { |data| -data[:stats][:agility] }.first(3),
      luck: stats_data.sort_by { |data| -data[:stats][:luck] }.first(3)
    }
    
    render json: {
      level: level,
      job_stats: stats_data,
      rankings: rankings.transform_values do |top_jobs|
        top_jobs.map { |job| { name: job[:name], value: job[:stats][rankings.key(top_jobs)] } }
      end
    }
  end

  # 複数レベルでの比較表示
  def multi_level_comparison
    levels = params[:levels] || [1, 10, 20, 30, 40, 50]
    levels = levels.map(&:to_i).select { |l| l > 0 && l <= 100 }
    job_ids = params[:job_ids] || JobClass.active.limit(4).pluck(:id)
